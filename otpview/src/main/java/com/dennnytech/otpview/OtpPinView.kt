package com.dennnytech.otpview
...
class OtpPinView @JvmOverloads constructor(
    ...
    private var maxCountPerLine: Int = 6
    ...
    private var changedListener: OTPChangedHandler? = null
    ...
    init {
        context.theme.obtainStyledAttributes(
            ...
            try {
                ...
                showCursor = getBoolean(R.styleable.OtpView_showCursor, true)
                isPassword = getBoolean(R.styleable.OtpView_isPassword, false)
                ...
                maxCountPerLine = getInt(R.styleable.OtpView_maxCountPerLine, 6).let {
                    if (it < 1) 6 else it
                }
            } finally {
                recycle()
    ...
    private fun addEditTexts(start: Int = 0) {
        if (autoSize) {
            ...
            for (i in start until squareCount) {
                ...
                val editText = OtpEditText(context).apply {
                    layoutParams =
    ...
    /**
     * A listener for OTP changes.
     */
    fun setOnChangedListener(listener: OTPChangedHandler) {
            ...
            currentEditText.setOnKeyListener { _, keyCode, event ->
                if (keyCode == KeyEvent.KEYCODE_DEL && event.action == KeyEvent.ACTION_DOWN) {
                    ...
                    if (currentEditText.text.isNotEmpty()) {
                        ...
                        if (index > 0) {
                            getPreviousEditText(index)?.requestFocus()
                        }
    ...
    inner class OtpEditText(context: Context) : AppCompatEditText(context) {
